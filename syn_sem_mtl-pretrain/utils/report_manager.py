...
def build_report_manager(opt, task_type):
    if opt.tensorboard:
    ...
    report_mgr = ReportMgr(opt.report_every, start_time=-1,
                           tensorboard_writer=writer, task_type=task_type)
    return report_mgr
...
class ReportMgrBase(object):
    ...
    def report_training(self, step, num_steps, learning_rate,
        ...
        if step % self.report_every == 0:
            ...
            return Statistics(task_type=self.task_type)
        else:
            return report_stats
...
class ReportMgr(ReportMgrBase):
    def __init__(self, report_every, start_time=-1., tensorboard_writer=None, task_type='task'):
        """
    ...
    def _report_training(self, step, num_steps, learning_rate,
        ...
        """
        # report_stats.output(self.task_type,
        #                     step, num_steps,
        ...
        self.maybe_log_tensorboard(report_stats,
                                   ...
                                   learning_rate,
                                   self.progress_step)
        report_stats = Statistics(self.task_type)
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        if train_stats is not None:
            self.log(self.task_type + ' Train perplexity: %g' % train_stats.ppl())
            self.log(self.task_type + ' Train accuracy: %g' % train_stats.accuracy())
        ...
        if valid_stats is not None:
            self.log(self.task_type + ' Step: %2d; Validation perplexity: %g' % (step, valid_stats.ppl()))
            self.log(self.task_type + ' Step: %2d; Validation accuracy: %g' % (step, valid_stats.accuracy()))
