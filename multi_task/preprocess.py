...
from inputters.dataset import get_fields, get_task_fields, get_task2_fields, build_dataset, make_text_iterator_from_file
...
def merge_vocabs(vocabs, vocab_size=None, min_frequency=1):
  ...
  return torchtext.vocab.Vocab(merged,
                               specials=[Constants.UNK_WORD, Constants.PAD_WORD,
                                         Constants.BOS_WORD, Constants.BOS_WORD2,
                                         Constants.EOS_WORD],
                               max_size=vocab_size,
...
def build_vocab(train_dataset_files, fields, share_vocab,
                ...
                tgt2_vocab_size, tgt2_words_min_frequency,
                vocab_file):
  counter = {}
  ...
  build_field_vocab(fields["tgt2"], counter["tgt2"],
                     max_size=tgt2_vocab_size,
                     min_freq=tgt2_words_min_frequency)
  
  merged_vocab = merge_vocabs(
        [fields["tgt"].vocab, fields["tgt2"].vocab],
...
def build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, fields,
  ...
  for index, src in enumerate(src_list):
    ...
    pt_file = "{:s}_{:s}_{:s}.{:d}.pt".format(
      opt.save_data, task_type, corpus_type, index)
...
def build_save_vocab(train_dataset, fields, opt):
  ...
  store_vocab_to_file(fields['tgt2'].vocab, opt.save_data + '_tgt2_vocab')
...
def build_save_dataset(corpus_type, task_type, fields, opt):
  ...
  logger.info(" * saving %s %s dataset to %s." % (task_type, corpus_type, pt_file))
  torch.save(dataset, pt_file)
