...
if __name__=="__main__":
  _, model_save_dir, ensemble_path , *checkpoints_list = argv
  if model_save_dir is None:
    print("model_save_dir error")
  
  print("Averaging checkpoints: \n{}".format(checkpoints_list))
 
  print("start average the model") 
  model_list = []
  generator_list = []
  for checkpoint_path in checkpoints_list:
    checkpoint = torch.load(model_save_dir + checkpoint_path)
  ...
  for key, value in model_dict.items():
    model_dict_avg[key] = value / number
  for key, value in generator_dict.items():
    generator_dict_avg[key] = value / number
